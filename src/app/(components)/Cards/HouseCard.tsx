...
const Card = ({ object }: { object: any }) => {
  // Extrair imagens válidas (não deletadas)
  const images = (object.accommodation_photo || [])
    .filter((img: any) => !img.deleted)
    .map((img: any) => img.photo)

  // Fallback para imagem vazia
  const fallbackImage = 'https://via.placeholder.com/400x250?text=Sem+Imagem'
      ...
      <figure className="w-full h-[250px] rounded-[20px] relative overflow-hidden group" title={object.title}>
        ...
        {images.length > 1 && (
          <>
            <button
              onClick={prevImage}
              className="absolute left-2 top-1/2 -translate-y-1/2 z-10 p-1 bg-white/70 rounded-full hover:bg-white transition cursor-pointer"
            >
              <ChevronLeft size={16} />
            </button>
            <button
              onClick={nextImage}
              className="absolute right-2 top-1/2 -translate-y-1/2 z-10 p-1 bg-white/70 rounded-full hover:bg-white transition cursor-pointer"
            >
      ...
      <div className="">
        ...
        <div className="flex items-end justify-between">
          <div className="font-medium text-[10px] leading-none tracking-normal capitalize text-[#999999]">
            ...
            <p className="mt-[2px]">{object.category_id}</p>
          </div>
